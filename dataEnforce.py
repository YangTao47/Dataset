...
def get_imgaug(data, img_path, img_id):
    ...
    seq = iaa.SomeOf(2, [
        ...
        iaa.ScaleX((0.8, 1.2)),
        iaa.ScaleY((0.8, 1.2)),
        iaa.Affine(scale={"x": (0.9, 1.1), "y": (0.8, 1.1)},
                   rotate=(-90, 90))], random_order=True)
    # 变换后的图片和box
    img_aug, bbs_aug, kps_aug = seq(image=img, bounding_boxes=bbs, keypoints=kps)
    cv2.imwrite("B:\\meter-data\\meter_seg\\2021-12-25-new\\" + str(idx + nums) + '.jpg', img_aug)
    # 绘制变换前box在图片上的位置
...
def write_json_file(data, path, bbs_aug, kps_aug):
    ...
    with open("B:\\meter-data\\meter_seg\\2021-12-25-new\\" + str(idx + nums) + '.json', 'w') as f:
